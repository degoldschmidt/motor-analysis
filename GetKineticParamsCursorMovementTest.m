%% Function to get kniematic parameters for a certain subject for the Test phase
function [TrajsTang, TargetAngle, Teach, Curl, ErrAng, NT, CutInds, ErrAngss] = GetKineticParamsCursorMovementTest(subj, nT)
 %our data has an empty first trial for all
subj.TrialDataXYTest = subj.TrialDataXYTest(2:end); 
subj.ShotDataXYTest = subj.ShotDataXYTest(2:end); 
subj.AngleShotTest=subj.AngleShotTest(2:end);
subj.AngleTargetTest=subj.AngleTargetTest(2:end);
subj.ForceFieldTest=subj.ForceFieldTest(2:end);
subj.TargetDataXYTest=subj.TargetDataXYTest(2:end);
%
CurlThr = 1.5;%10000;% Distribution thresholds
TimeThr = 2.3;%10000;% Distribution thresholds
TrajsT = subj.TrialDataXYTest;
% Calculate error angles
diff = mod(pi-cell2mat(subj.AngleShotTest),2*pi) - cell2mat(subj.AngleTargetTest);
ind = find(diff>pi);
...
RealTrajsT{1} = P;
for j = 2 : length(TrajsT)
    l = length(TrajsT{j-1}(:,1));
    X1 = TrajsT{j}(:, 1);
    X2 = TrajsT{j}(:, 2);
...
TargetAngle = cell2mat(subj.AngleTargetTest);
